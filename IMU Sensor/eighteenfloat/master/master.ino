...
void loop() {
  ...
  // 第一次传输
  radio.write(&dataToSend, 8 * sizeof(float));
  // 第二次传输
 radio.write(((uint8_t*) &dataToSend) + (8 * sizeof(float)), 8 * sizeof(float));
  // 第三次传输
 radio.write(((uint8_t*) &dataToSend) + (16 * sizeof(float)), 2 * sizeof(float)); 
  delay(16);
}